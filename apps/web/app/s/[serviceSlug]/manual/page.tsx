...
export async function generateMetadata({
  ...
  if (serviceSlug !== SENNOKUNI_SLUG) return { title: 'かんたんマニュアル' };
  return {
    title: '千ノ国メディア｜かんたんマニュアル',
    description: '千ノ国メディアの登録、初期設定、投稿、ポイント確認を順番に説明します。',
  };
}
...
export default async function SennokuniManualPage({
  params,
}: {
  params: Promise<{ serviceSlug: string }>;
}) {
  const { serviceSlug } = await params;
  if (serviceSlug !== SENNOKUNI_SLUG) notFound();
